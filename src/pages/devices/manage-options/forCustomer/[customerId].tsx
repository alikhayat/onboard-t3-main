...
import type { DeviceComponent, DeviceTemplate } from "@prisma/client";
...
export default function MspTemplateTablePage() {
  const router = useRouter();
  const { customerId } = router.query;
  const devicesQuery = trpc.device.getDeviceTemplatesByCustomerId.useQuery(
  ...
  if (devicesQuery.isError) {
    return <div>Error while loading devices</div>;
  }

  const labelText = devicesQuery?.data?.map((d) => {
    return {
      id: d.id,
      name: d.name,
    ...
    <CustomerLayout title={"Device Options"}>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-32">
                ...
                <li
                  onClick={() => {
                    if (!devicesQuery.data) {
                      ...
                      setSelectedTemplate(
                        devicesQuery?.data?.find((d) => d.id === device.id)
                      );
                    }
                  }}
                  key={device.id}
        ...
        <div className="text-center">
          <div className="mt-6">
            <button
              onClick={() => router.push(`/devices/new/${customerId}`)}
              type="button"
