...
const customerNavigation = [
  ...
  { name: "People", icon: UsersIcon, isMSPLink: false, path: "/people" },
  {
    name: "Device Options",
    icon: ComputerDesktopIcon,
    path: "/devices/manage-options",
    isMSPLink: false,
  },
];
...
export default function CustomerLayout(props: Props) {
  const router = useRouter();
  const { customerId } = router.query;
  const { children, title } = props;
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const { data: session, status } = useSession();
  const isMsp = session?.user?.accountType === "MSP";
  const navigation = isMsp ? mspNavigation : customerNavigation;
                    ...
                    <nav className="mt-5 space-y-1 px-2">
                      ...
                      {navigation.map((item) => (
                        <a
                          key={item.name}
                          onClick={() => {
                            if (item?.isMSPLink === true) {
                              router.push(`${item.path}/${customerId}`);
                            } else {
